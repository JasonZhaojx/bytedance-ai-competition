...
if TYPE_CHECKING:
    from extracted_core.product_workflow import ProductWorkflowResult
...
def _detect_product_type_by_keywords(result: ProductWorkflowResult) -> ProductType:
    ...
    # 扩展软件产品关键词列表
    software_keywords = [
        "软件", "应用", "app", "website", "平台", "系统", "程序", "服务", "插件",
        "客户端", "工具", "办公", "社交", "游戏", "编辑器", "浏览器", "通讯",
        "photoshop", "word", "excel", "ppt", "office", "微信", "qq", "钉钉",
        "淘宝", "京东", "拼多多", "抖音", "快手", "bilibili", "youtube",
        "chrome", "firefox", "edge", "safari", "vscode", "notepad", "sublime"
    ]
    hardware_keywords = [
        "手机", "电脑", "设备", "机器", "硬件", "笔记本", "台式机", "平板",
        "相机", "耳机", "音箱", "手表", "手环", "路由器", "电视", "冰箱", "空调",
        "小米", "华为", "苹果", "apple", "samsung", "sony", "联想", "dell",
        "路由器", "switch", "playstation", "xbox", "kindle"
    ]
    ai_tools_keywords = [
        "编程", "代码", "开发", "IDE", "copilot", "coding", "AI", "copilot",
        "智能", "大模型", "模型", "api", "插件", "tool", "assistant", "灵码",
        "codebuddy", "claude", "cursor", "kimi", "豆包", "通义", "文心"
    ]
    ...
    candidates_text = " ".join([
        c.title + " " + c.snippet + " " + " ".join(c.extracted_params.keys())
        for c in result.candidates
    ]).lower()
    all_text = product_name_lower + " " + summary_lower + " " + candidates_text

    # 统计各类关键词出现次数
    software_count = sum(1 for k in software_keywords if k in all_text)
    hardware_count = sum(1 for k in hardware_keywords if k in all_text)
    ai_tools_count = sum(1 for k in ai_tools_keywords if k in all_text)

    _log_soft = lambda msg: None  # 静默模式

    # 优先检查AI编程工具
    if ai_tools_count >= 2 or (ai_tools_count >= 1 and software_count > 0):
        return ProductType.AI_TOOLS
    if any(k in product_name_lower for k in ai_tools_keywords):
        return ProductType.AI_TOOLS
    ...
    # 检查 extracted_params 中的字段来判断（辅助）
    extracted_fields = set()
    for candidate in result.candidates:
        if hasattr(candidate, "extracted_params") and candidate.extracted_params:
            extracted_fields.update(candidate.extracted_params.keys())

    software_fields = {"developer", "platform", "pricing", "rating", "features", "supported_languages"}
    hardware_fields = {"brand", "model", "price", "spec", "battery", "weight", "dimensions"}

    extracted_software_score = len(extracted_fields & software_fields)
    extracted_hardware_score = len(extracted_fields & hardware_fields)

    if extracted_software_score > extracted_hardware_score:
        return ProductType.SOFTWARE
    if extracted_hardware_score > extracted_software_score:
        return ProductType.HARDWARE
...
def _get_domain_config(config: QualityConfig, result: ProductWorkflowResult) -> DomainConfig:
    ...
    if product_type == ProductType.AI_TOOLS:
        return DomainConfig.ai_tools()
    elif product_type == ProductType.SOFTWARE:
        return DomainConfig.software()
    else:
        return DomainConfig.hardware()


# ========== 证据质量评估 ==========

def _is_valid_candidate(candidate) -> bool:
    """检查候选是否有效（非导航链接）."""
    ...
    # 检查URL是否指向有效页面
    if ".json" in url or ".xml" in url or url.endswith(".js"):
        return False
...
def _evaluate_evidence_quality(
    ...
    # 检查是否是有效候选
    if hasattr(candidate_or_review, "url"):
        if not _is_valid_candidate(candidate_or_review):
            return EvidenceQualityScore(
                score=0.0,
                url_trusted=False,
                content_length_ok=False,
                structured_fields_ok=False,
                blocked=False,
                reasons=["无效候选：导航链接或页面"]
            )
...
try:
    ...
    def chat_content(api_key: str, base_url: str, model: str, messages: list, **kwargs):
        raise NotImplementedError(
            "chat_content function not found. Please ensure extracted_core.llm_client is available."
        )
