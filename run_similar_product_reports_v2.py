...
from report_agent.search_adapter import (  # noqa: E402
    ReportSearchConfig,
    SearchBundle,
    adapt_upstream_output_for_report,
)
...
def build_report_agent_adapter_config() -> ReportSearchConfig:
    """构造 search_adapter 的配置。

    v2 前半段已经按根脚本规则选好了 LLM provider。这里显式复用
    `final_llm_config()`，保证“上游总结”和“adapter 改写”默认走同一套模型。
    如需单独替换 adapter 模型，可用 REPORT_ADAPTER_LLM_* 环境变量覆盖。
    """

    config = ReportSearchConfig.from_env()
    api_key, base_url, model = final_llm_config()
    config.llm_api_key = os.getenv("REPORT_ADAPTER_LLM_API_KEY") or api_key
    config.llm_base_url = os.getenv("REPORT_ADAPTER_LLM_BASE_URL") or base_url
    config.llm_model = os.getenv("REPORT_ADAPTER_LLM_MODEL") or model
    config.progress_printer = print
    return config


def build_report_agent_writing_config() -> WritingAgentConfig:
    """构造 report_agent 核心写作链路的配置。

    和 adapter 分开配置，是因为 adapter 只负责“格式改写”，而
    `run_writing_agent()` 会继续做 evidence、insight、SWOT、报告生成。
    如果后续要给报告生成换更强模型，可以只设置 REPORT_LLM_*，不影响上游。
    """
    ...
    return WritingAgentConfig(
        ...
        llm_model=model,
        use_llm=env_bool(
            "REPORT_AGENT_USE_LLM",
            bool(api_key and base_url and model),
        ),
        temperature=float(os.getenv("REPORT_AGENT_TEMPERATURE", "0.2")),
        max_tokens=int(os.getenv("REPORT_AGENT_MAX_TOKENS", "3500")),
        llm_timeout=int(
            os.getenv("REPORT_AGENT_TIMEOUT", str(FINAL_SUMMARY_TIMEOUT))
        ),
        verbose=True,
    )


def build_report_agent_upstream_output(
    report_paths: list[Path],
    ...
    """把 v2 已经生成的上游产物打包成 search_adapter 的输入。

    这里不做事实抽取，也不改写报告内容；只是把上游输出完整交给
    `search_adapter`：
    - 单品报告：保留 FINAL SUMMARY、参考点和完整 Markdown。
    - 横向总结：保留最终汇总文件全文。
    - 问卷分析/参数关键词库：作为需求侧背景一起传入。

    后续“改成 report_agent 需要的数据格式”的工作统一放在
    `report_agent/search_adapter.py`。
    """

    single_reports = []
    for path in report_paths:
        # read_report_for_summary 会把单品报告拆成 final_summary 和参考点。
        # 同时保留 full_report，是为了让 adapter 需要时能回看完整上下文。
        item = read_report_for_summary(path)
        full_report = path.read_text(encoding="utf-8", errors="replace")
    ...
    return {
        "upstream_workflow": "run_similar_product_reports.py",
        "product_description": product_description,
        "selected_products": targets,
        "comparison_keyword_library": comparison_keyword_library,
        "questionnaire_analysis": {
            "path": questionnaire_analysis_path,
            "content": questionnaire_analysis_text,
        },
        "single_product_reports": single_reports,
        "final_comparison": {
            "title": "上游所选产品横向对比报告",
            "path": str(final_comparison_path),
            "content": final_comparison_path.read_text(
                encoding="utf-8", errors="replace"
            ),
            "source": "run_similar_product_reports",
            "content_source": "upstream_final_comparison",
        },
    }


def run_report_agent_workflow(
    report_paths: list[Path],
    final_comparison_path: Path,
    product_description: str,
    targets: list[str],
    ...
    """执行新增的中游 report_agent 工作流。

    完整链路为：
    上游输出 -> search_adapter 改写 -> core.py/run_writing_agent -> 文件输出。

    这个函数放在 v2 顶层编排里，只负责把已有产物串起来；真正的格式适配
    在 search_adapter，真正的报告生成在 core.py。
    """

    print("\n===== Report Agent 工作流 =====")

    # 1. 把前面上游流程产出的多个文件/文本打包为一个原始 payload。
    upstream_output = build_report_agent_upstream_output(
        report_paths,
    ...
    # 2. 用 search_adapter 的智能体把上游 payload 改写成 source 列表。
    #    返回的 bundle.results 会直接进入 report_agent/core.py。
    adapter_config = build_report_agent_adapter_config()
    bundle = adapt_upstream_output_for_report(
        upstream_output,
    ...
    if bundle.errors:
        for error in bundle.errors:
            print(f"[warn] search_adapter: {error}")
    if not bundle.results:
        raise RuntimeError("search_adapter 没有产出可供 report_agent 使用的 sources。")

    # 3. 调用 report_agent 主入口。这里不再适配下游，只产出 ReportPackage。
    writing_config = build_report_agent_writing_config()
    package = run_writing_agent(
        bundle.results,
        config=writing_config,
        task_id=f"{timestamp}_report_agent",
        analysis_goal=(
            f"基于上游竞品搜索、单品报告和横向总结，为 {product_description} "
            "生成结构化竞品分析报告"
        ),
        target_domain=product_description,
        competitors=targets,
    )

    # 4. 把最终 Markdown、结构化 package 和 adapter 中间结果都落盘，方便调试
    #    “上游输出 -> adapter sources -> report_agent 报告”的每一步。
    output_paths = write_report_agent_outputs(package, bundle, timestamp)
    print(f"Report Agent Markdown: {output_paths['report_markdown']}")
    print(f"Report Agent Package: {output_paths['report_package']}")
    print(f"Report Agent Sources: {output_paths['adapted_sources']}")
    return output_paths


def write_report_agent_outputs(
    package,
    bundle: SearchBundle,
    timestamp: str,
) -> dict[str, Path]:
    """落盘 report_agent 的三个关键产物。

    - REPORT_AGENT.md：最终给人读的 Markdown 报告。
    - REPORT_AGENT_PACKAGE.json：core.py 的完整结构化输出，可供下游质检读取。
    - REPORT_AGENT_ADAPTED_SOURCES.json：search_adapter 改写后的中间 sources，
      用来排查报告证据质量问题。
    """

    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    report_path = REPORT_DIR / f"{timestamp}_REPORT_AGENT.md"
    package_path = REPORT_DIR / f"{timestamp}_REPORT_AGENT_PACKAGE.json"
    sources_path = REPORT_DIR / f"{timestamp}_REPORT_AGENT_ADAPTED_SOURCES.json"

    # Markdown 和 package 是 report_agent 的正式输出。
    report_path.write_text(package.report_markdown, encoding="utf-8")
    package_path.write_text(package.to_json(), encoding="utf-8")

    # adapted sources 是中间调试产物：如果最终报告缺证据或证据粒度不对，优先
    # 查看这个文件，而不是回头改上游搜索。
    sources_payload = {
    ...
    sources_path.write_text(
        json.dumps(sources_payload, ensure_ascii=False, indent=2, default=str),
        encoding="utf-8",
    )
    return {
        "report_markdown": report_path,
        "report_package": package_path,
        "adapted_sources": sources_path,
    }


def env_bool(name: str, default: bool) -> bool:
    """读取布尔环境变量，支持 0/false/no/off/否/关闭 表示关闭。"""
...
def main() -> None:
    ...
    print(f"\n总总结已保存: {final_path}")
    report_agent_paths = run_report_agent_workflow(
        report_paths,
        final_path,
        product_description,
        targets,
    ...
    print(f"\nReport Agent 最终报告已保存: {report_agent_paths['report_markdown']}")
