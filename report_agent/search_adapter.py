...
class ReportSearchConfig:
    ...
    def from_env(cls) -> "ReportSearchConfig":
        ...
        return cls(
            ...
            timeout=20,
            workers=int(os.getenv("REPORT_AGENT_TABLE_GAP_SEARCH_WORKERS", "0")),
        )
...
def search_queries_for_report(
    ...
    *,
    ...
    if configured_workers <= 0:
        max_workers = len(clean_queries)
    else:
        max_workers = min(len(clean_queries), max(1, configured_workers))
    ...
    if max_workers <= 1 or len(clean_queries) == 1:
        query_results_by_index = [run_one(item) for item in enumerate(clean_queries, 1)]
    else:
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="report-gap-search") as executor:
            futures = [executor.submit(run_one, item) for item in enumerate(clean_queries, 1)]
